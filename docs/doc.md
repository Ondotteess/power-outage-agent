...
Система реализована как асинхронный event-driven pipeline на `asyncio`. Точка входа — `app/main.py`. Компоненты связаны через in-memory очередь `TaskQueue` и центральный `Dispatcher`, который роутит задачи по `TaskType`.
...
Не реализовано: dedup нормализованных событий, Office Matcher, Notifier, FastAPI-эндпоинты, Alembic-миграции.
...
│   │   ├── models.py             — Source, RawRecord, ParsedRecord, NormalizedEvent, TaskRecord
│   │   └── repositories.py       — stores для задач, raw, parsed, normalized, sources
│   ├── models/
...
Схема создаётся через `Base.metadata.create_all` при старте. Alembic пока не входит в runtime-зависимости; миграции надо добавлять отдельным шагом, когда схема перестанет быть экспериментальной.
...
Если handler для типа задачи не зарегистрирован, задача дропается с warning. Сейчас зарегистрированы `fetch_source`, `parse_content`, `normalize_event`.

### 7.2 CollectorHandler
...
## 10. Запуск и smoke-проверка

Подготовка:

```bash
cp .env.example .env
echo "LLM_API_KEY=..." > .env.local
...
Текущий счётчик: 88 pytest-кейсов, все зелёные.
...
- Нет dedup нормализованных событий.
- Нет Office Matcher и модели офисов.
- Нет Notifier, dashboard и FastAPI endpoints.
