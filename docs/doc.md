...
**Архитектурный слой (текущая итерация — конец Week 2):**
...
Scheduler ──submit──▶ Dispatcher ──▶ TaskStore (tasks: pending/running/done/failed)
                          ...
                          └─▶ ParseHandler (PARSE_CONTENT)
                                 ...
                                 │      (RossetiSibParser / RossetiTomskParser / …)
...
- ParseHandler с реестром парсеров (`rosseti_sib`, `rosseti_tomsk`).
...
| `parsed_records` | `id`, `raw_record_id (FK)`, `source_id (FK)`, `external_id`, `start_time`, `end_time`, `location_city`, `location_district`, `location_street`, `location_region_code`, `reason`, `extra (JSON)`, `trace_id`, `extracted_at` | Структурированные записи до нормализации         |
...
| Ключ                 | Тип    | Назначение                                                                                                  |
| -------------------- | ------ | ----------------------------------------------------------------------------------------------------------- |
| `parser`             | str    | Имя парсера в реестре (`rosseti_sib`, `rosseti_tomsk`)                                                      |
